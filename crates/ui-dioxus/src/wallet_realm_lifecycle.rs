...
use oxid_wallet_application::{
    ...
    WalletRealmLifecycleInput, WalletRealmLifecycleSettlement, WalletRealmLifecycleStatus,
...
pub(super) async fn explicit_retry(
    ...
    let result = lifecycle
        .execute(WalletRealmLifecycleInput::ActionPreflight {
            now_millis: monotonic_millis(),
...
async fn drive_selected_realm(services: WalletUiServices, profile_id: String, resumed: bool) {
    ...
    if execute_with_timeout(Arc::clone(&lifecycle), input)
        .await
        .is_err()
    {
        return;
    ...
    loop {
        ...
        let Some(next_wakeup) = status.next_wakeup_millis else {
            std::future::pending::<()>().await;
            return;
        };
        let now_millis = monotonic_millis();
        tokio::time::sleep(Duration::from_millis(
            next_wakeup.saturating_sub(now_millis).max(1),
        ))
        .await;
