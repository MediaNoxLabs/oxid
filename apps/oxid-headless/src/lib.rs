...
impl HeadlessWallet {
    ...
    fn dispatch(&self, line: &str) -> Dispatch {
        ...
        match request.method.as_str() {
            ...
            "wallet.realm.sync.status" => self.selected_realm_sync_status(request),
            "wallet.realm.sync.start" => self.start_selected_realm_sync(request),
            "wallet.realm.sync.cancel" => self.cancel_selected_realm_sync(request),
