...
use oxid_wallet_application::{
    WalletAccountQuery, WalletRealmFacetState, WalletRealmLifecycleIdentity,
    WalletRealmLifecycleInput, WalletRealmReconciliationState,
};
...
impl HeadlessWallet {
    ...
    pub(super) fn drive_selected_realm(&self, profile_id: String) -> Result<(), ()> {
        ...
        block_on(lifecycle.execute(input))
            .map(|_| ())
            .map_err(|_| ())
    }
