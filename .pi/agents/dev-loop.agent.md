...
For a reducer, coordinator, scheduler, or other asynchronous state machine, the implementation checkpoint also requires a compact command/event property matrix. Cover the applicable completion, recovery, supersession or replacement, cancellation, stale-event, and duplicate-event cells with tests or an explicit impossibility argument. Produce this before external review so the reviewer does not discover the state space one event permutation at a time.
