...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "The iOS developer-profile smoke requires macOS and Xcode." >&2
  exit 1
fi
...
OXID_IOS_DEVICE="$device" OXID_IOS_RESET_DATA=1 OXID_UI_PROFILE=dev \
...
env -i \
  ...
  -only-testing:OxidUITests/DeveloperProfileTests/testBannerAndSharedCapabilityManifestRemainVisible \
  CODE_SIGNING_ALLOWED=NO

echo "iOS standalone developer-profile manifest smoke passed on $device."
