...
test("target recipes preserve run and expose receipt-gated build and deploy modes", async () => {
  ...
  assert.match(justfile, /^desktop-build:\n    cargo build -p oxid-app$/m);
  assert.match(justfile, /^desktop-run:\n    cargo run -p oxid-app$/m);
  assert.match(
    justfile,
    /^desktop-live-build:\n    cargo build --profile desktop-live -p oxid-app --no-default-features --features desktop,standalone-development,standalone-local$/m,
  );
  assert.match(
    justfile,
    /^desktop-live-run:\n    cargo run --profile desktop-live -p oxid-app --no-default-features --features desktop,standalone-development,standalone-local$/m,
  );
