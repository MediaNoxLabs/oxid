...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "The iOS demo-profile smoke requires macOS and Xcode." >&2
  exit 1
fi
...
OXID_IOS_DEVICE="$device" OXID_IOS_RESET_DATA=1 OXID_UI_PROFILE=demo \
...
env -i \
  ...
  -only-testing:OxidUITests/DemoProfileTests/testFullSetupStopsAtExistingCredentialOfferReview \
  CODE_SIGNING_ALLOWED=NO

echo "iOS standalone demo-profile bootstrap smoke passed on $device."
