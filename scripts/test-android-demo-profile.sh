...
if [ -n "$device" ]; then
  OXID_ANDROID_DEVICE="$device" OXID_UI_PROFILE=demo \
    "$repository_root/scripts/run-android-emulator.sh"
else
  OXID_UI_PROFILE=demo "$repository_root/scripts/run-android-emulator.sh"
  device="$($adb_command devices | awk 'NR > 1 && $2 == "device" { print $1; exit }')"
fi
if [ -z "$device" ]; then
  echo "The Android demo-profile harness did not find an online device." >&2
  exit 1
fi

echo "Resetting Oxid application data on Android device $device for the demo-profile smoke."
...
node "$repository_root/tests/mobile/android-wallet-flow.mjs" "$websocket_url" demo

echo "Android standalone demo-profile bootstrap smoke passed on $device."
