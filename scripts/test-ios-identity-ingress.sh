...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "The iOS identity-ingress test requires macOS and Xcode." >&2
  exit 1
fi
...
if [ ! -x /usr/bin/xcodebuild ] || [ ! -x /usr/bin/xcrun ]; then
  echo "Xcode is required for the iOS identity-ingress test." >&2
  exit 1
fi
...
generated_project_root="$repository_root/target/mobile-tests/ios-identity-ingress"
...
test_source="$repository_root/tests/mobile/ios/OxidUITests/IdentityIngressTests.swift"
test_names=()
...
if [ "${#test_names[@]}" -eq 0 ]; then
  echo "No IdentityIngressTests methods were discovered in $test_source." >&2
  exit 1
fi

for test_name in "${test_names[@]}"; do
  echo "Running isolated iOS identity-ingress scenario: $test_name"
  ...
  env -i \
    ...
    -derivedDataPath "$repository_root/target/mobile-tests/ios-identity-ingress-derived-data" \
    -only-testing:"OxidUITests/IdentityIngressTests/$test_name" \
    CODE_SIGNING_ALLOWED=NO
done

echo "iOS simulator QR-unavailable and warm/cold custom-scheme ingress passed on $device."
