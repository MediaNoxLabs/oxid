...
| `wallet-core/vault` | Passport-vault contract interaction and selective-disclosure claim | `passport-vault/domain`, `passport-vault/application`, product adapter, not generic wallet core | ADR-0051 delivers exact standalone multi-lock behavior; ADR-0052 adds the immutable five-circuit artifact closure and native tagged-state decoding; ADR-0054/0055 authenticate canonical history/state; ADR-0056/0057 add the retained call harness and explicit simulator; ADR-0058 authenticates the generated client/four wallet proof circuits; ADR-0059 through ADR-0062 add closed-schema create/deposit/withdraw composition, exact finalized public context, protected NIGHT input/change signing, and zeroizing funded native drafts while claim custody, DUST proving/submission, and durable state remain issue #31 |
...
does not expose raw circuit arguments. ADR-0060 installs the composer behind a
native retained application-port adapter. That path requires canonical replay
...
remains unavailable, so claim composition, DUST proof provision, durable
journaling, node submission, and reconciliation are the next issue #31
increments.
...
capability discovery reports `native_funded_draft` while
`settlesOnMidnight` remains false. Protected claim composition, DUST balancing,
proving, submission, durable public journaling, and optional state caching
remain issue #31 adapter work. WebView JavaScript, iframe origins, hard-coded
addresses, and relative workspace paths remain excluded.
