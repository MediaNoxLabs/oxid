...
| `wallet-core` address, HD, balances, transaction, sync | Midnight addresses, derivation, NIGHT/DUST, build/sign/submit, indexer/node access | chain-neutral chain domain/use cases plus `adapters/midnight` | Network/account reads, simulated/live sync, durable public unshielded plus private DUST/Zswap checkpoint/resume, protected NIGHT/DUST/Zswap receive derivation, native shielded replay, fresh-sync-gated shielded spend, and staged public/private transfer through DUST/Zswap proof, safe pre-broadcast cancellation, and finalized node inclusion implemented for standalone/headless/mobile; ADR-0098/#91 prove funded unshielded and genesis-authority shielded headless finality/adapter-reconstruction flows and add signed-profile plus node-genesis production gates; ADR-0099 implements the distinct protected DUST-registration repository/headless boundary, while funded preprod registration/recovery, durable production custody, provisioned deployment, funded mobile flows, and physical-device proof budgets remain gated |
...
ADR-0099 records the registration boundary implemented in Oxid.
...
repository/headless slice is implemented, including explicit consent and
transfer-journal separation. A fresh wallet intentionally begins with zero
DUST; funded preprod registration-to-generation/recovery, Dioxus/mobile,
restart, physical-device, and production results remain open.
