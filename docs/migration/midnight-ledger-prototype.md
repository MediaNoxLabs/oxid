...
| `wallet-core/vault` | Passport-vault contract interaction and selective-disclosure claim | `passport-vault/domain`, `passport-vault/application`, product adapter, not generic wallet core | ADR-0051 delivers exact standalone multi-lock behavior; ADR-0052 adds the immutable five-circuit artifact closure and native tagged-state decoding; ADR-0054/0055 authenticate canonical history/state; ADR-0056/0057 add the retained call harness and explicit simulator; ADR-0058 authenticates the generated client/four wallet proof circuits; ADR-0059/0060 add closed-schema create/deposit/withdraw composition plus zeroizing retained native drafts while fresh Midnight context, claim custody, funding/proving/submission, and durable state remain issue #31 |
...
prepare/authorize behavior. The composition root does not yet provide its
fresh Midnight context, and submit remains unavailable, so combined
contract/DUST proof provision, NIGHT funding, durable journaling, node
submission, and reconciliation are the next issue #31 increments.
