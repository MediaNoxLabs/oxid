...
| `wallet-core/secret_storage` and `unlock` | Multi-curve keys, encrypted files, redb, opaque references, boot lock, attempt throttling | wallet-owned session/key-operation ports plus platform-backed and development adapters | ADR-0017/0046 accepted; process-local Ed25519/P-256/Jubjub plus BIP32/secp256k1-Schnorr conformance and standalone selected-DID issuance binding implemented; presentation-time holder reauthorization, durable recovery, and native custody pending |
