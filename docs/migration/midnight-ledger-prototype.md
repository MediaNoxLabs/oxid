...
| `wallet-core` address, HD, balances, transaction, sync | Midnight addresses, derivation, NIGHT/DUST, build/sign/submit, indexer/node access | chain-neutral chain domain/use cases plus `adapters/midnight` | Network/account reads, simulated/live sync, durable public unshielded plus private DUST/Zswap checkpoint/resume, protected NIGHT/DUST/Zswap receive derivation, native shielded replay, fresh-sync-gated shielded spend, and staged public/private transfer through DUST/Zswap proof, safe pre-broadcast cancellation, and node inclusion implemented for standalone/headless/mobile; ADR-0098 proves a funded unshielded headless finality/restart flow and adds signed-profile plus node-genesis production gates, while durable production custody, provisioned deployment, funded mobile flows, and physical-device proof budgets remain gated |
...
recipient, proves finalized inclusion and public-journal restart
reconciliation, and waits boundedly for a stable recipient read. Live evidence
