...
| `wallet-core/vault` | Passport-vault contract interaction and selective-disclosure claim | `passport-vault/domain`, `passport-vault/application`, product adapter, not generic wallet core | ADR-0051 delivers exact standalone multi-lock behavior; ADR-0052 adds the immutable five-circuit artifact closure and native tagged-state decoding; ADR-0054/0055 authenticate canonical history/state; ADR-0056/0057 add the retained call harness and explicit simulator; ADR-0058 authenticates the generated client and four wallet proof circuits at runtime while live composition/funding/submission and durable standalone state remain issue #31 |
...
through the wallet capability. This does not yet compose or settle a call: the
generated client sidecar, combined contract/DUST proof provider, NIGHT funding,
node submission, and durable reconciliation are the next issue #31 increments.
