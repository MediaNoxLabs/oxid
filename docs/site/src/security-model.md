...
content-addressed Nix builds, cargo-audit/cargo-deny gates, and a canonical
rev-pinned `arrayref` source after the 2026-08-20 crates.io publication mismatch.
The gate also rejects the unreviewed `proc-macro1` dependency introduced only
by that registry publication. A dated, per-advisory exception register lives under
