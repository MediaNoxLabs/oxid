...
| Solana MWA, local | Android intent → `ws://localhost` | **No** | No |
...
| MITM on the local transport | Application-layer AEAD terminated **inside the Rust core**, keyed out-of-band. Bonded BLE data is readable by all apps on the device; a plaintext `ws://` LAN channel carrying witness material would violate ADR-0027's non-loopback rule. Never rely on link-layer security. |
