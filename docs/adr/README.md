...
| [0099](0099-register-protected-dust-keys-before-fresh-wallet-submission.md) Register protected DUST keys before fresh-wallet submission | Accepted | §§3–8, 12–13, 16–18, 21; prototype onboarding plan; ledger registration semantics; issues #2/#92 | Implemented in repository/headless/Dioxus with guarded public PreProd funding-manifest/read-only-observation foundations and an amount-observed one-output/one-note acceptance harness: a distinct prepare/consent/authorize/submit port registers only current owned NIGHT inputs through protected role-0/role-2 custody; a fresh wallet intentionally starts with zero DUST, and authoritative generated-DUST observation remains a separate spend-readiness gate |
