...
| [`crates/composition/src/profile_in_memory.rs`](../../crates/composition/src/profile_in_memory.rs) and its tests | The explicit in-memory application composition used by many tests | Product composition seam: composition tests call the public façade and pass only lower-level public values to testkit helpers; testkit never calls, replaces, owns, or copies the composition root |
