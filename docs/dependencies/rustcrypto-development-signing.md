...
- Cryptography: deterministic Ed25519 signatures and RFC 6979 P-256 ECDSA
  signatures; only high-level signing APIs are enabled. Hazardous, key import,
  PKCS#8, PEM, batch, ECDH, and legacy-compatibility features are not enabled.
...
- Adapter boundary: `crates/adapters/storage-dev`; domain/application crates do
  not depend on cryptography libraries
