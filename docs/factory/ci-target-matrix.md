...
| `unit-linux` | `./run.sh unit` (workspace core/headless/MCP units; UI/app excluded to their lane) | x86_64 Linux, minimal `ci-rust` shell, sccache | core, UI, headless, mobile/platform, Compact, build | 5-10 min command target; hard 12 min job cap includes setup, metrics, and cleanup; historical cold workspace tests 119 s locally | Hosted PR lane |
