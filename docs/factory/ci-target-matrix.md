...
| `coverage-linux` | `./run.sh coverage` | minimal `ci-coverage` shell and `cargo-llvm-cov`; the workspace scope explicitly enables deployment readiness, Tailnet DID publication, and development wallet fixture host-test features for production modules otherwise selected only by device profiles | every Rust-affecting area | hard 25 min, 80% line floor; changed production files without an instrumented mapping fail closed and are reported together, while conventional `src/**/tests.rs`, `*_tests.rs`, and the reviewed declaration-only composition facade remain explicitly non-executable | Hosted PR lane |
