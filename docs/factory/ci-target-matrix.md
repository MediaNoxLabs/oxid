...
| `coverage-linux` | `./run.sh coverage` | minimal `ci-coverage` shell and `cargo-llvm-cov`; the workspace scope explicitly enables reviewed host-test features for production modules otherwise selected only by device profiles | every Rust-affecting area | hard 25 min, 80% line floor; changed production files without an instrumented mapping fail closed | Hosted PR lane |
